...
public final class OrderService {
    ...
    public OrderService(SessionManager session, TableService tables, MenuService menu) {
    ...
    public List<Ticket> closedCheckHistoryForCurrentWaiter() {
        Employee u = session.currentUser();
        if (u == null || u.role() != Role.WAITER) return List.of();
    ...
    public void removeGhostDraftTicketsForTable(String tableId) {
        ...
        activeTickets().removeIf(t -> t.tableId().equalsIgnoreCase(tableId) && isGhostDraftTicket(t));
        persist();
    }
    ...
    public Ticket openOrCreateForTable(String tableId) {
        ...
        requireClockedIn();
        RestaurantTable t = tables.requireTable(tableId);
        if (!t.isAssignedTo(u.employeeId())) {
        ...
        removeGhostDraftTicketsForTable(tableId);

        ObservableList<Ticket> list = activeTickets();
        Optional<Ticket> existing = list.stream()
                .filter(x -> x.tableId().equalsIgnoreCase(tableId))
        ...
        Ticket created = new Ticket(u.employeeId(), tableId, orderNo, guests);
        list.add(created);
        persist();
        return created;
    }
    ...
    public Optional<Ticket> latestTicketForTable(String tableId) {
        ...
        return activeTickets().stream()
                .filter(t -> t.tableId().equalsIgnoreCase(tableId))
    ...
    private Employee requireWaiter() {
        ...
        if (u.role() != Role.WAITER) throw new IllegalStateException("Only waiters can manage orders");
        return u;
    }
